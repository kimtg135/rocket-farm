...
# EKS Admin Role (for access entry)
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks_oidc_provider.url, "https://", "")}:sub" = "system:serviceaccount:karpenter:karpenter"
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          ...
          "arn:aws:ssm:*:*:parameter/${var.project_name}/${var.region_role}/*"
        ]
      },
      {
        Effect = "Allow"
